...
export class GameSearch {
    ...
    displayHeaderSearchResults(games) {
        ...
        if (!games || games.length === 0) {
            ...
            let content = '';
            const mostPopular = games.slice(0, 1);
            const otherMatches = games.slice(1, 4);

            if (mostPopular.length > 0) {
                content += '<div class="search-category-title">САМОЕ ПОПУЛЯРНОЕ</div>';
                mostPopular.forEach(game => content += this.createSearchResultItem(game));
            }

            if (otherMatches.length > 0) {
                content += '<div class="search-category-title">ЛУЧШИЕ СОВПАДЕНИЯ</div>';
                otherMatches.forEach(game => content += this.createSearchResultItem(game));
            }
            
            if (games.length > 4) {
                 content += `<a href="#" class="search-view-all">Результаты поиска <i class="fas fa-arrow-right ml-2"></i></a>`;
            }
