...
export class AuthManager {
    ...
    async handleRegistration() {
        ...
        try {
            ...
            if (newUser) {
                toastManager.success(`Регистрация прошла успешно!`);
                this.userManager.updateUIForLoggedInUser(email);
                modalManager.hideModal(Utils.getElement('auth-modal'));
                window.location.reload();
            } else {
                toastManager.error('Не удалось создать пользователя');
            }
     ...
     */
    setupChangePassword() {
        const changePasswordForm = Utils.getElement('change-password-form');
        if (!changePasswordForm) return;

        const newPasswordInput = Utils.getElement('new-password');
        const policyLength = Utils.getElement('change-policy-length');
        const policyLower = Utils.getElement('change-policy-lower');
        const policyDigit = Utils.getElement('change-policy-digit');

        if (newPasswordInput && policyLength && policyLower && policyDigit) {
            Utils.addEventListenerSafe(newPasswordInput, 'input', () => {
                this.updatePasswordPolicy(newPasswordInput.value, policyLength, policyLower, policyDigit);
            });
        }

        Utils.addEventListenerSafe(changePasswordForm, 'submit', (e) => {
            e.preventDefault();
    ...
    async handleChangePassword() {
        ...
        if (!currentUser) {
            toastManager.error('Вы должны быть авторизованы, чтобы изменить пароль');
            return;
        }

        const currentPassword = Utils.getElement('current-password').value;
        const newPassword = Utils.getElement('new-password').value;
        const confirmNewPassword = Utils.getElement('confirm-new-password').value;
        ...
        if (!this.userManager.verifyPassword(currentUser.email, currentPassword)) {
            toastManager.error('Текущий пароль неверен');
            return;
        }

        const passwordValidation = Utils.validatePassword(newPassword);
        if (!passwordValidation.isValid) {
            toastManager.error('Новый пароль не соответствует требованиям безопасности');
            return;
        }

        if (newPassword !== confirmNewPassword) {
            toastManager.error('Новые пароли не совпадают');
            return;
        }

        try {
            this.userManager.updateUserPassword(currentUser.email, newPassword);
            toastManager.success('Пароль успешно изменен');
            modalManager.hideModal(Utils.getElement('change-password-modal'));
            Utils.getElement('change-password-form').reset();
        } catch (error) {
            console.error('Change password error:', error);
            toastManager.error('Ошибка при смене пароля');
        }
    }
