    ...
    setupNotificationHistory() {
        ...
        if (notificationHistoryBtn && notificationHistoryModal) {
            const toggleModal = (show) => {
                const isVisible = notificationHistoryModal.classList.contains('visible');
                if (show && !isVisible) {
                    notificationHistoryModal.classList.remove('hidden');
                    requestAnimationFrame(() => {
                        notificationHistoryModal.classList.add('visible');
                    });
                    this.moveToastsToModal();
                    this.loadNotificationHistory();
                } else if (!show && isVisible) {
                    notificationHistoryModal.classList.remove('visible');
                    notificationHistoryModal.addEventListener('transitionend', () => {
            ...
            Utils.addEventListenerSafe(document, 'click', (e) => {
                const isVisible = notificationHistoryModal.classList.contains('visible');
                if (isVisible && !notificationHistoryModal.contains(e.target) && !notificationHistoryBtn.contains(e.target)) {
                    toggleModal(false);
                }
            });
        }
